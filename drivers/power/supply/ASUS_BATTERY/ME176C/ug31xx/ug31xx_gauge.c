...
static bool early = false; /* Register power supply early? */
...
//Webber [TT-559041] ------
	...
	{
		val->intval = ug31->batt_charge_now * 1000;
	}
	if (psp == POWER_SUPPLY_PROP_CHARGE_FULL)
	{
		val->intval =ug31->batt_charge_full * 1000;
	}
...
// 20141024 Tom -------
	...
	if(!early && ug31xx_powersupply_init(ug31->client))
	{
		goto pwr_supply_fail;
	}
...
MODULE_PARM_DESC(early, "Register power supply early (may not be fully initialized yet)");
