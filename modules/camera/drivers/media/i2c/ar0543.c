...
#include <linux/proc_fs.h> //ASUS_BSP, Read OTP value
...
static char camera_module_otp[60]; //ASUS_BSP+++, Read OTP value
...
static ssize_t ar0543_read_otp(struct device *dev,struct device_attribute *attr,char *buf) //ASUS_BSP, Read OTP value
{
...
static DEVICE_ATTR(ar0543_read_otp, S_IRUGO, ar0543_read_otp, NULL); //ASUS_BSP, Read OTP value
...
//static int otp_proc_read(struct seq_file *m, char **start, off_t off, int count, int *eof, void *data)
// Fix declaration to avoid build warning and error.
...
{
        proc_file = proc_create(OTP_PROC_FILE, 0666, NULL, &otp_proc_ops);
        if (!proc_file)
...

{
        ...
        //printk("%s: dev->fmt_idx %d frame_length 0x%X\n", __FUNCTION__, dev->fmt_idx, frame_length);
...
static int ar0543_init_registers(struct v4l2_subdev *sd)
{
        ...
        vcm_i2c_wr8(client, 0x02, 0x15);
        vcm_i2c_wr8(client, 0x03, 0xA4);
        vcm_i2c_wr8(client, 0x06, 0x00);
        vcm_i2c_wr8(client, 0x07, 0xc8);
        ...
        dev_err(&client->dev, "ar0543_init_registers error");
        ...
        return -1;
retry:
        dev_err(&client->dev, "ar0543_init_registers error => retry");
        ret = __ar0543_s_power(sd, 0);
        msleep(30);
        if (ret) {
            dev_err(&client->dev, "ar0543 power down err");
        ...
        // add return to avoid build warning and error.
        return ret;
}
...
static int __ar0543_init(struct v4l2_subdev *sd, u32 val)
{
        ...
        if (!dev->keeps_focus_pos) {
                v4l2_warn(sd, "VCM does not maintain focus pos in standbymode, using sw workaround\n");
        }
                              ...
                              struct v4l2_mbus_framefmt *fmt)
{
        ...
        pr_info("%s: after __get_resolution_index width:%d  height:%d  dev->fmt_idx:%d\n",__func__, fmt->width, fmt->height, dev->fmt_idx);
        /* Sanity check */
                                       ...
                                       struct v4l2_frmivalenum *fival)
{
        ...
        pr_info("%s: final fival->width %d fival->height %d index %d\n",__func__, fival->width, fival->height, index);
                            ...
                            int irq, void *pdata)
{
        ...
        if (ret) {
                v4l2_err(client, "ar0543_detect err s_config 1.\n");
                msleep(100);
                ret = ar0543_detect(client, &sensor_id, &sensor_revision);
                if(ret)
                {
...
static int ar0543_add_dvfs(void)
{
        struct dentry *rul;
        struct dentry *debugfs_root;
        debugfs_root = debugfs_create_dir("camera0", NULL);
        rul = debugfs_create_u8("camera_status", 0644, debugfs_root, &atd_node);
        if (!rul)
                goto Fail;
        return 1;
