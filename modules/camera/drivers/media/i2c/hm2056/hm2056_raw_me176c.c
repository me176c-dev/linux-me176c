...
#include "../i2c_seqops/i2c_seqops.h"

#include "hm2056_raw_me176c.h"
#include <linux/seq_file.h> //ASUS_BSP, Read OTP value
...
struct v4l2_subdev *main_sd[2] = {NULL, NULL};

int ATD_hm2056_raw_status[2] = {0, 0};
//static char camera_module_otp[2][60] = {"", ""};
...
*/
	return sprintf(buf,"%s", camera_module_otp[client->addr == 0x24? 0 : 1]);
}
#endif
...
static const char *g_i2c_seqops_pkgs[] =
{
	"hm2056_stream_on",
	"hm2056_stream_off",
	"hm2056_init",
	"hm2056_init_flip",
	"hm2056_uxga_14fps",
	"hm2056_uxga_14fps_flip",
	"hm2056_hdp_19fps",
	"hm2056_hdp_19fps_flip",
	"hm2056_1392x780_28fps",
	"hm2056_1392x780_28fps_flip",
	"hm2056_1296x812_28fps",
	"hm2056_1296x812_28fps_flip",
	"hm2056_720p_27fps",
	"hm2056_720p_27fps_flip",
	"hm2056_svga_30fps",
	"hm2056_svga_30fps_flip",
	NULL,
};

//ASUS_BSP+++
struct _i2c_seqops_entry_t *g_i2c_seqops;

DEFINE_I2C_SEQOPS_ENTRY(g_i2c_seqops_front, "i2cseq_hm2056.bin", g_i2c_seqops_pkgs);
DEFINE_I2C_SEQOPS_ENTRY(g_i2c_seqops_back, "i2cseq_hm2056_b.bin", g_i2c_seqops_pkgs);
//ASUS_BSP---
...
static int hm2056_raw_set_suspend(struct v4l2_subdev *sd)
{
	...
	ret = i2c_seqops_exec(g_i2c_seqops, "hm2056_stream_off"); //ASUS_BSP
	if (ret)
		return -EINVAL;
	return 0;
}
...
static int hm2056_raw_set_streaming(struct v4l2_subdev *sd)
{
	...
	ret = i2c_seqops_exec(g_i2c_seqops, "hm2056_stream_on"); //ASUS_BSP
	if (ret)
		return -EINVAL;
...
static int power_up(struct v4l2_subdev *sd)
{
	...
	//dev_info(&client->dev, "%s client->name(%s) client->dev.init_name(%s) client->dev.type->name(%s) client->dev.driver->name(%s)\n",
	//	__func__,client->name,client->dev.init_name,client->dev.type->name,client->dev.driver->name);
	if(strcmp(client->name,"hm2056f") == 0 )
	{
		unsigned short slaveaddr = client->addr;
		client->addr = 0x24;
		dev_info(&client->dev, "%s client->addr(0x%x) slaveaddr(0x%x)\n", __func__,client->addr,slaveaddr);

		ret = hm2056_raw_write_reg(client, HM2056_8BIT, 0xFF00, 0x5A);
		if (ret) {
			dev_err(&client->dev, "Failed to Write 0xFF00, 0x5A\n");
			goto fail_clk;
		}else{
			dev_info(&client->dev, "%s Write 0xFF00, 0x5A ret(%d)\n", __func__,ret);
		}

		ret = hm2056_raw_write_reg(client, HM2056_8BIT, 0xFF00, 0xA5);
		if (ret) {
			dev_err(&client->dev, "Failed to Write 0xFF00, 0xA5\n");
			goto fail_clk;
		}else{
			dev_info(&client->dev, "%s Write 0xFF00, 0xA5 ret(%d)\n", __func__,ret);
		}

		ret = hm2056_raw_write_reg(client, HM2056_8BIT, 0xFF00, slaveaddr);
		if (ret) {
			dev_err(&client->dev, "Failed to Write 0xFF00, 0x%02x\n",slaveaddr);
			goto fail_clk;
		}else{
			dev_info(&client->dev, "%s Write 0xFF00, 0x%02x ret(%d)\n", __func__,slaveaddr,ret);
		}
		client->addr = slaveaddr;
	}
...
static int hm2056_raw_s_power(struct v4l2_subdev *sd, int power)
{
	...
	if (power == 0) {
		ret = power_down(sd);
                ...
                i2c_seqops_reloadfw(g_i2c_seqops, 0); //ASUS_BSP
                i2c_seqops_updateclient(g_i2c_seqops, client); //ASUS_BSP
		ret = power_up(sd);
		if (!ret) {
					
			//workaround for i2c error
			msleep(10);			
			
			if(strcmp(client->name,"hm2056f") == 0 ){
				printk("%s write hm2056_init_flip\n",__func__);
				//<ASUS-Lew-20140212+>
				//ret = hm2056_raw_write_reg_array(client, hm2056_init_flip);
				while (retry++ < 3){
				        //ret = i2c_seqops_exec(&g_i2c_seqops, "hm2056_init_flip");
                                        ret = i2c_seqops_exec(g_i2c_seqops, "hm2056_init_flip");//ASUS_BSP
					if (ret == 0) break;
					pr_err("%s, init failed, retry: %d\n",__func__, retry);
					power_down(sd);
					msleep(10);
					power_up(sd);
				}
			}else{
				printk("%s write hm2056_init\n",__func__);
				//<ASUS-Lew-20140212+>
				//ret = hm2056_raw_write_reg_array(client, hm2056_init); 
				//ret = i2c_seqops_exec(&g_i2c_seqops, "hm2056_init");
				while (retry++ < 3){
					ret = i2c_seqops_exec(g_i2c_seqops, "hm2056_init"); //ASUS_BSP
					if (ret == 0) break;
					pr_err("%s, init failed, retry: %d\n",__func__, retry);
					power_down(sd);
					msleep(10);
...
{
	struct hm2056_raw_device *dev = to_hm2056_raw_sensor(sd);
	enum v4l2_mbus_pixelcode pixel_order = V4L2_MBUS_FMT_SBGGR8_1X8;
	...
	if (!strncmp(sd->name, "hm2056b", strlen("hm2056b"))) {
	        pixel_order = V4L2_MBUS_FMT_SGBRG8_1X8;
	}
	fmt->code = pixel_order;
	mutex_unlock(&dev->input_lock);
			      ...
			      struct v4l2_mbus_framefmt *fmt)
{
	...
	//ret = hm2056_raw_write_reg_array(client, dev->hm2056_raw_res[dev->fmt_idx].regs);
        ret = i2c_seqops_exec(g_i2c_seqops, dev->hm2056_raw_res[dev->fmt_idx].regs); //ASUS_BSP
...
static int hm2056_raw_detect(struct hm2056_raw_device *dev, struct i2c_client *client)
{
	...
	ATD_hm2056_raw_status[client->addr == 0x24 ? 0 : 1] = 0;	//Add for ATD command+++
	...
	ATD_hm2056_raw_status[client->addr == 0x24 ? 0 : 1] = 1;	//Add for ATD command+++
...
hm2056_raw_s_config(struct v4l2_subdev *sd, int irq, void *platform_data)
{
	...
	if (ret) {
		v4l2_err(client, "hm2056_raw_detect err s_config 1.\n");
		msleep(100);
		ret=hm2056_raw_detect(dev,client);
		if(ret)
		{
			v4l2_err(client,"hm2056_raw_detect err s_config 2.\n");
			goto fail_detect;
		}
	}
			...
			struct v4l2_streamparm *param)
{
	...
	switch (dev->run_mode) {
		case CI_MODE_VIDEO:
			if(strcmp(client->name,"hm2056f") == 0 ){
				printk("%s write hm2056_raw_res_video_flip\n",__func__);
				dev->hm2056_raw_res = hm2056_raw_res_video_flip;
				dev->n_res = N_RES_VIDEO_FLIP;
			}else{
				printk("%s write hm2056_raw_res_video\n",__func__);
		...
		default:
			if(strcmp(client->name,"hm2056f") == 0 ){
				printk("%s write hm2056_raw_res_still_flip\n",__func__);
				dev->hm2056_raw_res = hm2056_raw_res_still_flip;
				dev->n_res = N_RES_STILL_FLIP;
			}else{
				printk("%s write hm2056_raw_res_still\n",__func__);
...
static int hm2056_raw_remove(struct i2c_client *client)
{
	...
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
        ...
        i2c_seqops_clear(g_i2c_seqops); //ASUS_BSP
		       ...
		       const struct i2c_device_id *id)
{
	...
	if (!dev) {
        ...
        if(strncmp(id->name, HM2056B_NAME, 7) == 0){
            printk("ASUS_BSP++ %s hm2056b \n",__func__);
            g_i2c_seqops = &g_i2c_seqops_back;//ASUS_BSP++ for seperate front and back
        }else{
            printk("ASUS_BSP++ %s hm2056f \n",__func__);
            g_i2c_seqops = &g_i2c_seqops_front; //ASUS_BSP++ for seperate front and back
        }
	...
	v4l2_i2c_subdev_init(&dev->sd, client, &hm2056_raw_ops);
        ...
        i2c_seqops_init(g_i2c_seqops, client); //ASUS_BSP
	pr_info("%s: i2c seqops init 0x%x\n", __func__, client->addr);
	...
	main_sd[client->addr == 0x24 ? 0 : 1] = &dev->sd;	//Add for ATD command+++

	return ret;
}
#if 1
MODULE_DEVICE_TABLE(i2c, hm2056b_raw_id);

static struct i2c_driver hm2056b_raw_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = HM2056B_NAME,
	},
	...
	.id_table = hm2056b_raw_id,
};

static __init int init_hm2056b_raw(void)
{
	return i2c_add_driver(&hm2056b_raw_driver);
}

static __exit void exit_hm2056b_raw(void)
{
	i2c_del_driver(&hm2056b_raw_driver);
}

module_init(init_hm2056b_raw);
module_exit(exit_hm2056b_raw);
#endif
// ASUS_BSP+++, need to check
#if defined(CONFIG_ME176C)
MODULE_DEVICE_TABLE(i2c, hm2056f_raw_id);

static struct i2c_driver hm2056f_raw_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = HM2056F_NAME,
	},
	.probe = hm2056_raw_probe,
	.remove = hm2056_raw_remove,
	.id_table = hm2056f_raw_id,
};

static __init int init_hm2056f_raw(void)
{
	return i2c_add_driver(&hm2056f_raw_driver);
}

static __exit void exit_hm2056f_raw(void)
{
	i2c_del_driver(&hm2056f_raw_driver);
}

module_init(init_hm2056f_raw);
module_exit(exit_hm2056f_raw);
#endif
