...
DEFINE_I2C_SEQOPS_ENTRY(g_i2c_seqops, "i2cseq_hm2056.bin", g_i2c_seqops_pkgs);
...
static int hm2056_raw_set_suspend(struct v4l2_subdev *sd)
{
	...
	ret = i2c_seqops_exec(&g_i2c_seqops, "hm2056_stream_off");
	if (ret)
		return -EINVAL;
	return 0;
}
...
static int hm2056_raw_set_streaming(struct v4l2_subdev *sd)
{
	...
	ret = i2c_seqops_exec(&g_i2c_seqops, "hm2056_stream_on");
	if (ret)
		return -EINVAL;
...
static int hm2056_raw_s_power(struct v4l2_subdev *sd, int power)
{
	...
	if (power == 0) {
		...
		i2c_seqops_reloadfw(&g_i2c_seqops, 0);
		i2c_seqops_updateclient(&g_i2c_seqops, client);
		ret = power_up(sd);
		if (!ret) {
			      ...
			      struct v4l2_mbus_framefmt *fmt)
{
	...
	ret = i2c_seqops_exec(&g_i2c_seqops, dev->hm2056_raw_res[dev->fmt_idx].regs);
		       ...
		       const struct i2c_device_id *id)
{
	...
	i2c_seqops_init(&g_i2c_seqops, client);
