...
//		printk("%s: fp->f_dentry->d_inode->i_size=%d\n", __FUNCTION__, bootbin_size);
		...
		if(fp->f_op != NULL && fp->f_op->read != NULL){
			...
			for (i = 0; i < (bootbin_size/7) + 1; i++) {
				...
				ret |= gc2155_write_reg(client, GC2155_8BIT, Reg,Val);
			}
				  ...
				  struct v4l2_mbus_framefmt *fmt)
{
	...
	case GC2155_RES_CIF:
		pr_info("gc2155_set_mbus_fmt: CIG\n");
					...
					struct atomisp_exposure *exposure)
{
	...
	//Todo:Short term workaround,need to be fixed
	...
	ret = gc2155_write_reg(client, GC2155_8BIT, 0xb1, DigitalGain);
	ret = gc2155_write_reg(client, GC2155_8BIT, 0x25, AnalogGain);
						...
						struct v4l2_frmivalenum *fival)
{
	...
	if (i == N_RES)
		i--;
	pr_info("%s(%d): fival: {wxh}={%dx%d}\n", __func__, __LINE__,fival->width,fival->height);
	index = i;
	pr_info("%s(%d): gc2155_res[%d]: {wxh}={%dx%d}\n", __func__, __LINE__,index,gc2155_res[index].width,gc2155_res[index].height);
...
#if 0
static int gc2155_dbgfs_init(int proj_id)
{
...
static int gc2155_add_dvfs(void)
{
	struct dentry *rul;
	struct dentry *debugfs_root;
	int project_id = asustek_boardinfo_get(FUN_PROJECT_ID);
	printk("%s : get camera sku =%d",__func__,project_id);
	if ((project_id == ME176CX_L) || (project_id == ME176CE)) {
		debugfs_root = debugfs_create_dir("camera0", NULL);
		rul = debugfs_create_u8("camera_status", 0644, debugfs_root, &atd_node);
		if (!rul)
			goto Fail;
	}
	if (project_id == ME176C_L) {
		debugfs_root = debugfs_create_dir("camera1", NULL);
		rul = debugfs_create_u8("vga_status", 0644, debugfs_root, &atd_node);
		if (!rul)
			goto Fail;
	}
	return 1;
				...
				const struct i2c_device_id *id)
{
	...
	case PROJECT_ID_FE375CXG:
	{
		pr_info("%s(): cam_rear_id= 0x%x\n",
				__func__, cam_rear_id);
		if ((cam_rear_id == 2) || (cam_rear_id == 3)) {
			pr_info("gc2155 doesn't exist. Ignore %s()\n",
				__func__);
